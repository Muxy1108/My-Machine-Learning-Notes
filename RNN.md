# Common RNN Categories (by Architecture)

> Scope: This note classifies RNNs by **architecture** (structure).  
...
## 0) Two Orthogonal Axes (clarify the hierarchy)

### A) Architecture (what this note focuses on)
...
- **Vanilla RNN cell**: \( h_t = \tanh(W_x x_t + W_h h_{t-1} + b) \)
- **LSTM cell**: gated memory cell \(c_t\)
- **GRU cell**: update/reset gates

**Key implication:** “BiRNN / BiLSTM / BiGRU” are parallel names because they are
**the same architecture (bidirectional) + different cell types**.
...
Tasks where a token/frame label depends on **both left and right context**, e.g.
sequence tagging (POS/NER), offline speech frame classification, offline text encoding.

### 2.3 Key limitation (frequently tested)
Not suitable for strict real-time/online settings because the backward pass requires future inputs.

### 2.4 Why “BiRNN / BiLSTM / BiGRU” are written in parallel
- “Bi-” = the **bidirectional architecture**
- “RNN / LSTM / GRU” = the **cell type** inside each direction  
So:
- **BiRNN** = bidirectional + Vanilla cell
- **BiLSTM** = bidirectional + LSTM cell
- **BiGRU** = bidirectional + GRU cell
...
### 3.3 Pros / cons and common training tricks
...
### 4.1 Problem setting
Maps an input sequence \(x_{1:T}\) to an output sequence \(y_{1:U}\) (lengths may differ), e.g.
machine translation, summarization, dialogue generation, early attention-based ASR.
...
Decoder:
\[
h_u^{dec}=f(y_{u-1},h_{u-1}^{dec},c),\quad
p(y_u\mid y_{<u},x)=\text{softmax}(W h_u^{dec})
\]
...
### 4.4 “Then to Transformer” (what it means)
...
## 5) Quick “memorization” summary
- **Unidirectional (Vanilla)**: simplest; online-friendly; weak long-range memory  
- **Bidirectional**: uses left+right context; mainly offline; not strict real-time  
- **Stacked/Deep**: more expressive; harder/slower to train  
- **Seq2Seq**: sequence-to-sequence; Attention fixes the fixed-vector bottleneck; Transformer generalizes further
